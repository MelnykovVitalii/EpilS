...
window.addEventListener("load", event => {
  const prices = [depilacjaWoskiem, depilacjaLaserowa, elektroepilacja, kosmetologia, manicure, pedicure, rzęsy, brwi, promocijnePakiety];
  ...
  function getFemaleMaleList(femaleMale) {
    femaleMale.forEach(list => {
      ...
      div.classList = list.header ? "content__wrapper content-header" : "content__wrapper";
      const name = document.createElement("li");
      name.classList = list.bold ? "content__item bold" : "content__item name";
      // name.innerText = list.name;
      name.innerHTML = "";
      const arrayOfWords = list.name.split(" ");
      arrayOfWords.forEach(word => {
        name.innerHTML += `<span>${word}&nbsp</span>`;
      });
      const dots = document.createElement("li");
      dots.classList = list.price === -1 ? "content__item" : "content__item dots";
      const price = document.createElement("li");
      price.classList = "content__item price";
      price.innerHTML = `<span>${list.price === -1 ? "" : list.price + " zl"}</span>`;
      div.append(name);
      div.append(dots);
      div.append(price);
...
const arrowBottom = document.querySelector(".scroll-down-btn");
...
const HEADER_HEIGHT = headerWrapper.getBoundingClientRect().height;
...
menuList.addEventListener("click", event => {
  ...
  if (target && target.classList.contains("menu__link")) {
    ...
    const elementTop = document.getElementById(idBlock).getBoundingClientRect().height;
    let distance = document.getElementById(idBlock).getBoundingClientRect().top + window.pageYOffset;
    if (idBlock === "gift_card") {
      distance = distance - 60 - Math.abs((window.innerHeight - elementTop) / 2);
    }
    window.scrollTo({
      top: distance - HEADER_HEIGHT,
      behavior: "smooth"
    });
  }
});
...
arrowBottom.addEventListener("click", () => {
  const distance = scrollTarget.getBoundingClientRect().top + window.pageYOffset - HEADER_HEIGHT;
  window.scrollTo({
    top: distance,
    behavior: "smooth"
  });
...
window.addEventListener("scroll", () => {
  const arrayofBlocks = ["about", "zabiegi", "benefits", "effects", "feedback", "gift_card", "footer"];
  if (window.pageYOffset < header.getBoundingClientRect().height) {
    document.querySelector(`[data-anchor='about']`).classList.remove("active-item");
  }
  arrayofBlocks.forEach(elementId => {
    ...
    let isCloseToBlock = Math.trunc(elementTop) - HEADER_HEIGHT <= 2;
    if (elementId === "gift_card") {
      const giftCardTop = Math.abs((window.innerHeight - elementHeight) / 2);
      isCloseToBlock = elementTop - giftCardTop <= 2 + HEADER_HEIGHT + 60;
    }
    if (elementId === "footer") {
      const distanceToBottom = document.body.scrollHeight - window.pageYOffset - window.innerHeight;
      isCloseToBlock = Math.trunc(Math.abs(distanceToBottom)) === 0;
    }
    if (isCloseToBlock) {
      const allHeaderElements = document.querySelectorAll(`[data-anchor]`);
      allHeaderElements.forEach(menuItem => {
